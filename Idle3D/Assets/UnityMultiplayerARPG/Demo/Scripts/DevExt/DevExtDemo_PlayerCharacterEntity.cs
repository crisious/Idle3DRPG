...
namespace MultiplayerARPG
{
    public partial class PlayerCharacterEntity
    {
        ...
        protected void DevExtReceiveDamageDemo(HitBoxPosition position, Vector3 fromPosition, IGameEntity attacker, Dictionary<DamageElement, MinMaxFloat> allDamageAmounts, CharacterItem weapon, BaseSkill skill, short skillLevel)
        {
        ...
        protected void DevExtReceivedDamageDemo(HitBoxPosition position, Vector3 fromPosition, IGameEntity attacker, CombatAmountType combatAmountType, int damage, CharacterItem weapon, BaseSkill skill, short skillLevel, CharacterBuff buff)
        {
            if (writeAddonLog) Debug.Log("[" + name + "] PlayerCharacterEntity.ReceivedDamage("
                + attacker.GetGameObject().name + ", " + combatAmountType + ", " + damage + ")");
        }

        protected void DevExtApplyBuff(int dataId, BuffType type, short level, EntityInfo applierInfo)
        {
